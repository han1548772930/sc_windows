//! D2D/DWrite Factory 全局共享管理
//!
//! 提供 ID2D1Factory 和 IDWriteFactory 的全局单例，避免重复创建昂贵的 COM 对象。
//!
//! # 使用方式
//! ```ignore
//! use crate::platform::windows::factory::SharedFactories;
//!
//! // 获取共享的 Factory 实例
//! let factories = SharedFactories::get();
//! let d2d_factory = factories.d2d_factory();
//! let dwrite_factory = factories.dwrite_factory();
//! ```
    ...
    fn create() -> Result<Self> {
        ...
        unsafe {
            ...
            if result.is_err() && result != RPC_E_CHANGED_MODE {
                return Err(Error::new(windows::Win32::Foundation::E_FAIL, "COM initialization failed"));
            }
        }
...
mod tests {
    ...
    fn test_shared_factories_singleton() {
        let f1 = SharedFactories::try_get();
        let f2 = SharedFactories::try_get();
