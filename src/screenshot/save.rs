...
impl SaveManager {
    ...
    pub fn save_to_file(
        ...
        // 写入位图数据
        file.write_all(&rgb_data).map_err(|e| {
            ScreenshotError::SaveError(format!("Failed to write image data: {}", e))
        })?;
    ...
    /// 复制到剪贴板
    pub fn copy_to_clipboard(&self, screenshot: &ScreenshotData) -> Result<(), ScreenshotError> {
